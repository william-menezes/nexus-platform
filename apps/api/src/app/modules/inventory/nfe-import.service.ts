...
export class NfeImportService {
  ...
  async importXml(
    tenantId: string,
    xml: string,
  ): Promise<{ imported: number; created: number; matched: number }> {
    ...
    if (parsedItems.length === 0) return { imported: 0, created: 0, matched: 0 };
    ...
    // Novos produtos importados via NF-e são do tipo 'part' (peça) por padrão
    const newDtos = parsedItems
      .filter((i, idx, arr) => i.cProd && !productMap.has(i.cProd) && arr.findIndex(x => x.cProd === i.cProd) === idx)
      .map(i => ({
        tenantId, name: i.xProd, externalRef: i.cProd,
        type: 'part' as const,
        costPrice: i.cost, salePrice: i.cost, minStock: 0, currentStock: 0,
      }));
    ...
    return { imported: parsedItems.length, created: newDtos.length, matched };
  }
